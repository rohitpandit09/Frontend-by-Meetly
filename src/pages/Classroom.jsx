...
const Classroom = () => {
  ...
  useEffect(() => {
    const fetchClass = async () => {
      try {
        ...
        setClassData({
          ...data.class,
          members: (data.class.students || []).map((student) => ({
  ...
  useEffect(() => {
    ...
    const handleMembersUpdated = ({ memberSubmissions }) => {
      setClassData((prev) => ({
        ...prev,
        members: (prev.members || []).map((m) => ({
          ...m,
          submissionCount: memberSubmissions?.[m.id] || 0
        }))
      }));
    };
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          ...
          {activeTab === "assignments" && (
            <div className="space-y-4">
                ...
                <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
                  <div className="bg-white p-6 rounded-xl w-80 space-y-4">
                    ...
                    <div className="flex justify-end gap-2">
                      ...
                      <button
                        onClick={() => handleSubmit(activeAssignment)}
              ...
              {(classData?.assignments?.length || 0) === 0 ? (
                <div className="bg-card rounded-2xl border border-border p-12 text-center">
                  <FileText className="w-12 h-12 text-muted-foreground/30 mx-auto mb-3" />
                  <p className="text-muted-foreground">No assignments yet</p>
                </div>
              ) : (classData?.assignments || []).map((a, index) => (
                <div key={a._id || index} className="bg-card rounded-2xl border border-border p-6">
